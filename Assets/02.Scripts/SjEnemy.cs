...
public class SjEnemy : MonoBehaviour
{
    [Header("Enemy Stats")]
    ...
    #region Player Collision Damage
    ...
    private void CheckAndDamagePlayer(GameObject obj)
    {
        // 플레이어 레이어 확인
        int layer = obj.layer;
        ...
        // Combatant를 통해 데미지 적용
        var combatant = obj.GetComponent<Combatant>();
        if (combatant != null && combatant.CanTakeDamage)
        {
            combatant.TakeDamage(damageToPlayer);
            lastDamageTime = Time.time;

            Debug.Log($"[SjEnemy] {gameObject.name}이(가) {obj.name}에게 {damageToPlayer} 데미지를 입혔습니다. " +
                     $"남은 체력: {combatant.CurrentHealth:F1}/{combatant.MaxHealth:F1}");
        }
    }
